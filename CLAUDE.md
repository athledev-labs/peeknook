...
5. **Local-first inference, user-triggered capture.** Inference defaults to local Ollama (`http://127.0.0.1:11434`); a remote Ollama URL and Ollama `:cloud` tags are opt-in and HTTPS-gated (plain HTTP to a non-loopback host is rejected unless the user enables "Allow insecure HTTP" — see `OllamaURLPolicy` and PRIVACY.md). Do not add an inference path that bypasses the user's configured Ollama endpoint or this HTTPS gate. No ambient or background capture. Capture runs only when the user triggers it — by a capture/camera hotkey, or by **arming a Live session**, whose recurring refresh is itself a user act of arming. An armed session is **always indicated** (persistent Live chip + Stop) and **always bounded**: it disarms on every exit (Stop, New chat, switch/delete chat, collapse, hide, switch-away) and, when the optional extended-lifetime setting (`liveMaxArmedSeconds > 0`) is on, on a **mandatory inactivity timeout the user cannot disable** (`LivePolicy.expiresAt`, snapshot at arm, reset on interaction, enforced via `LiveRefreshPolicy.decide`'s `.expire` decision routed through `stopLiveSession()`). There is no silent, indefinite, or unindicated capture; `liveMaxArmedSeconds == 0` (the default) is byte-identical to no cap. The default flow skips the confirm step. Captures are stored in the conversation thread (`ChatTurn.image`), and History shows the full timeline. Opt-in `previewBeforeInfer` surfaces `appName` and `windowTitle` before analysis. Conversation persistence is **opt-in and off by default** (`PeeknookSettings.persistConversation`): when enabled, every answered chat, screenshots included, is filed as its own thread in a local **conversation archive** (`ConversationArchiveStore`: one `<uuid>.json` per thread plus an `index.v2.json`, under `Application Support/Peeknook/Conversations/`, capped at 25 threads / ~250 MB, oldest pruned first). Past chats are listable/resumable/deletable via the History switcher; discarding a chat (**New chat**) deletes just that thread, **Done** keeps it archived, and turning the setting off purges the whole archive. A one-time migration upgrades the legacy single-file `conversation.v1.json` (`ConversationStore`, kept only as the legacy reader). Never persist captures without that opt-in.
6. **Do not embed Noru.** Noruflow remains a separate product. A `NoruCaptureProvider` calls it as a **sidecar over a versioned loopback host-API contract** (CLI or HTTP), written on the Peeknook side against that published spec behind the `CaptureProviding` seam. Do not link Noru Rust/Tauri code into Peeknook. Any same-Apple-Team coupling (for example a shared App Group for zero-config discovery) is a **first-party convenience only** — it can never be the path a third-party consumer depends on, so it must never be *required*, and its entitlements must not land in a release build without a deliberate decision. Do not let a cross-repo agent commit Noru integration into this tree; the consumer's client lives in the consumer repo. Do not block Peeknook work on Noru integration.
...
- **Warm model latency.** `keep_alive` keeps the model resident (roughly 13s cold start to sub-second warm inference). `SessionOrchestrator.modelLikelyWarm` gates loading copy on real warm state (in-session timer **or** Ollama `/api/ps` residency after relaunch). Do not fake "Analyzing…" timers.
