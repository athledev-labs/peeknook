...
final class SuggestionCoordinator {
    ...
    func fetchSuggestions(gatedBy turnProfile: GroundProfile) {
        ...
        // assembles through the SAME shared seam (`assembleRequest`) at the `.primaryVision` role —
        // which resolves to the active answer model + endpoint, the identity pair the answer turn uses.
        // That makes the remote-egress redaction rule (a remote host or an Ollama `:cloud` tag)
        // un-forkable: a secret the answer turn stripped can't slip out on the immediately following
        // suggestion call. The hit count needs no notice — the answer pass already surfaced it.
        let assembled = session.assembleRequest(role: .primaryVision, quickMode: session.settings.quickMode) { redaction in
            builder.inferenceMessages(from: session.conversation, policy: .suggestions, redaction: redaction)
        }
